<fieldset>
    <div class="filtro_izquierdo_asignar">
        ...
        if (!empty($es_nuevo)) {
            echo $this->Form->input('es_nuevo',array('val'=>$es_nuevo,'type'=>'hidden'));
        }
        ...
        $disabled_consultar = true;
        if (!empty($this->data) && $this->data['Pago']['parametros'] == '1') {
           $disabled = false;
            $disabled_consultar = false;
        } else {
            $disabled = true;
        }
        ?>
        <fieldset class="fieldset_asignar_confirmacion" id="field_parametros" style="float:left">
            ...
            <table class="table_asignar_cartera">
                ...
                <tr>
                    ...
                    <td>Cedula o Rif</td>
                    <td><?php
                        echo $this->Form->input('cedula_desde',array('label'=>false,'disabled'=>$disabled));
                        ?>
                    </td>
                    <td><?php
                        echo $this->Form->input('cedula_hasta',array('label'=>false,'disabled'=>$disabled));
                        ?>
                    </td>
                </tr>
                <tr>
...
//                            $value = $this->data['Pago']['producto_id'];
//                            echo $value;
//                        } else {
//                           $value = 0;
//                        }
                    ...
                    <td>Monto</td>
                    <td><?php
                        echo $this->Form->input('monto_desde',array('label'=>false,'disabled'=>$disabled));
                        ?>
                    </td>
                    <td><?php
                        echo $this->Form->input('monto_hasta',array('label'=>false,'disabled'=>$disabled));
                        ?>
                    </td>
                </tr>
                <tr>
                    ...
                    <td>D/Mora</td>
                    <td><?php
                        echo $this->Form->input('mora_desde',array('label'=>false,'disabled'=>$disabled));
                        ?>
                    </td>
                    <td><?php
                        echo $this->Form->input('mora_hasta',array('label'=>false,'disabled'=>$disabled));
                        ?>
                    </td>
                </tr>
            </table>
        ...
        echo $this->Form->input('traspaso',array(
            ...
            'label'=> false,
            'class' => 'check_asignar',
            'style' => array('margin-top:70px'),
        ));
        if (!empty($this->data) && $this->data['Pago']['traspaso'] == '1') {
            $disabled = false;
            $disabled_consultar = false;
        } else {
            $disabled = true;
        ...
        $disabled_consultar = true;
        if (!empty($this->data) && $this->data['Pago']['directa'] == '1') {
            $disabled = false;
            $disabled_consultar = false;
        } else {
            $disabled = true;
        ...
        ?>
        <fieldset class="fieldset_asignar_confirmacion" id="field_directa" style="height:58px;">
        ...
        <fieldset class="fieldset_asignar_confirmacion orden" style="height:58px;">
            <legend>Orden</legend>
            <?php
            echo $this->Form->radio('tipo_orden',array('ASC' => 'Asc','DESC' => 'Desc'),array('legend'=>false));
            echo '&nbsp&nbsp&nbsp&nbsp|&nbsp&nbsp&nbsp&nbsp';
            echo $this->Form->radio('campo_orden',array('fecha' => 'Fecha','producto'=>'Producto','monto' => 'Monto','mora' =>'Mora' ),array('legend'=>false));
            ?>
        </fieldset>
        <div class="filtro_derecho_asignar" style="float:right">
            <?php
            echo $this->Form->submit('Consultar',array('disabled' => $disabled_consultar,'id' => 'consultar_asignacion'));
            if(!empty($this->data)) {
...
}elseif (!empty($this->data) && $this->data['Pago']['traspaso'] == '1') {
    if (!empty($deudores)) { ?>
        <table>
            <tr>
                <th>Cedula o Rif</th>
            ...
            foreach ($deudores as $n) {
                ?>
                <tr>
                    <td><?php echo $n['ClienProd']['CEDULAORIF'] ?></td>
                    <td><?php echo $n['Cobranza']['CONDICION'] ?></td>
                    <td><?php echo $n['Cobranza']['FECH_ASIG'] ?></td>
                    <td><?php echo $n['ClienProd']['PRODUCTO'] ?></td>
                    <td><?php echo $n[0]['SUM(`ClienProd`.`MONTO_SALV`)'] ?></td>
                    <td><?php echo $n['ClienProd']['DIASMORA'] ?></td>
                    <td><?php
                        echo $this->Form->input('deudores',array(
                            'name' => 'deudores_seleccionados['.$n['ClienProd']['CEDULAORIF'].']',
                            'type' => 'checkbox',
                            'label' => false,
                            'class' => 'check_deudores'
                        ));
...
}elseif (!empty($this->data) && $this->data['Pago']['directa'] == '1') {
    if (!empty($deudores)) {
        if ($es_nuevo) {
            ?>
            <table>
...
<script>
    ...
    $('#seleccionar_deudores').change(function() {
        if ($(this).is(':checked')) {
            $('check_deudores').attr('checked',true);
        } else {
            $('check_deudores').attr('checked',false);
        }
    });
